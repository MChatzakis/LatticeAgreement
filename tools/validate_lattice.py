...
for i in range(num_procs):
    if(i < 10):
        filename = "output-stress/proc0" + str(i+1)
    if(i >= 10):
        filename = "output-stress/proc" + str(i+1)
    with open(filename+".output") as f:
        decisions.append([])
        for line in f.readlines():
            decisions[i].append(set([int(x) for x in line.split(' ')]))
    
    with open(filename+".config") as f:
        configs.append([])
        f.readline()
        for line in f.readlines():
            configs[i].append(set([int(x) for x in line.split(' ')]))
...
for decision_idx in range(mindecided):
    for d1 in range(num_procs):
        for d2 in range(num_procs):
            ...
            if not ok:
                print("error in decision ", decision_idx)
